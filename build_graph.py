...
#quote_truths = truths[truths["is_quote"] == "t"].copy()
...
quote_activity = quotes_df.merge(
    truths,
    left_on=["quoted_truth_external_id", "quoted_user"],
    right_on=["truth_id", "user_id"],
    how="inner"
)
...
interaction_feats[qt_mask, 0] = 1.0
interaction_feats[re_mask, 1] = 1.0
...
# Create original posts DataFrame
...
# ----------------------------
print("Building quote activity...")
# Use only the first merge (more permissive)
quote_activity = quotes_df.merge(
    truths_df,
    left_on="quoted_truth_external_id", 
    right_on="truth_id",
...
# ----------------------------
# Filter out quotes and replies to get original posts
original_posts = truths_df[
    (truths_df["is_quote"] == "f") & 
    (truths_df["is_reply"] == "f")
].copy()

# Create POST activity: user posts their own content
...
post_activity["target_user"] = post_activity["engager"]  # self-posting
post_activity["interaction"] = "POST"
...
# Active user criteria (adjusted for three interaction types)
active_mask = (
...
# Cap to 20K for trainability (prioritize QT and RE over POST)
...
# Authorship edges (post → user) - for ALL post types
authorship = activity_sub[["post_id", "target_user"]].copy()
authorship["post_id"] = authorship["post_id"].map(post_to_idx)
authorship["target_user"] = authorship["target_user"].map(user_to_idx)
authorship = authorship.dropna().astype(int)
...
interaction_feats[qt_mask, 0] = 1.0
interaction_feats[re_mask, 1] = 1.0
interaction_feats[post_mask, 2] = 1.0
